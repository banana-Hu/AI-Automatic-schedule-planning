...
class SchedulerService {
  static Future<bool> checkAndReschedule(EventRepo repo) async {
    try {
      ...
      for (final event in expiredEvents) {
        ...
        if (!event.isArchived && eventEndTime < now) {
          ...
          // 查找第一个足以容纳该过期任务时长的空隙
          final gapStartTime =
              findFirstAvailableGap(futureEvents, event.durationMin, now);
  ...
  /// 查找第一个可用的时间空隙
  static int? findFirstAvailableGap(
      List<Event> futureEvents, int durationMin, int currentTime) {
    ...
    for (int i = 0; i < futureEvents.length - 1; i++) {
      ...
      final currentEndTime =
          currentEvent.startAt + currentEvent.durationMin * 60 * 1000;
